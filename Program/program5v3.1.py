...
while choice != len(optionList):
    if choice == 1:
        ...
        wordstoAdd = ValidateWord(wordstoAdd)
        ...
        if wordstoDelete.lower() in mywordDict.keys():
            del mywordDict[wordstoDelete.lower()]
        else:
            print("'",wordstoDelete,"'","does not exist.")
        print(PrintReport(mywordDict,"Word"))
    elif choice == 3:
        addPunc = input("Enter Punctuation: ")
        addPunc = ValidatePunc(addPunc)
