...
def ReadFromFile():
    """
    Ask user to select a file to populate Dictionary.
    """
    root = tk.Tk()
    root.withdraw()
...
def getValidNumber(userInput):
    ...
    """
    while not (userInput.isdigit() and int(userInput)>0):
        userInput = input("Only Positive Integer!. Enter valid number:")
...
def addEmployee(empDict):
    ...
    while getValidName(name) == False:
        print("Invalid Name!Must be First name and last name seperated by space.")
        name = input("Enter Full Name: ")
...
def PrintReport(empDict):
    ...
    nameList.sort(key = lambda name: name[1])  #Learned from W3School 
    newNameList = []
    for item in nameList:
        name = item[0] + " " + item[1]
...
#########################################main#######################################################################
"""
It will print the menu and ask user for choice. The menu can add employee , print the report and quit
"""
Dict = {}
ReadFromFile()
print(Dict)
